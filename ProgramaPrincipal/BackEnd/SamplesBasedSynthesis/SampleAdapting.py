...
def change_speed(wav_data, factor): 
	indices = np.round(np.arange(0, len(wav_data), factor)) #create an even-spaced array (wav_data) spaced by the factor. Ex. if factor was 2.333 it would change (1, 2, 3, 4, etc.) to (0, 2.333, 4.666 etc.)
...
def time_stretch(wav_data, factor, DFT_size, hop_size):
	...
	signal_out = np.zeros(int(L/factor) + DFT_size, dtype = complex)
	...
	while p < L - (DFT_size + hop_size):
		...
		p1 = int(p)
		spectra_1 = np.fft.fft(hanning_window * wav_data[p1: p1 + int(DFT_size)])
		spectra_2 = np.fft.fft(hanning_window * wav_data[p1 + int(hop_size): p1 + int(DFT_size) + int(hop_size)])
		...
		signal_out[pp:pp+int(DFT_size)] += (hanning_window * np.fft.ifft(np.abs(spectra_2)*out)) #using overlapping and blending - provides a smooth transition between two neighboring windows
		pp += int(hop_size)
		p += hop_size*factor
