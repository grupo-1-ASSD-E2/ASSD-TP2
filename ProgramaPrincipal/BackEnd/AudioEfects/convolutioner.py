...
class Convolutioner:
    ...
    def __init__(self, IR_left=np.ones(1), IR_right=np.ones(1), input_files=[], input_rate=44100,
                 input_dtype=np.float32, frame_count=2 ** 15):
        ...
        self.leftover_left = np.zeros((self.tracks_num, self.frame_count + self.M_left - 1),
                                      dtype=self.input_array.dtype)
        self.leftover_right = np.zeros((self.tracks_num, self.frame_count + self.M_right - 1),
                                       dtype=self.input_array.dtype)
    ...
    def reshape_IR(self, IR):
        if IR.ndim == 1:
            ...
            IR_delta = np.hstack((1 * np.ones((deltas_to_add, 1), dtype=self.input_array.dtype),
                                  np.zeros((deltas_to_add, IR.shape[1] - 1), dtype=self.input_array.dtype)))
    ...
    def pyaudio_callback(self, in_data, frame_count, time_info, status):
        ...
        audio_frame = np.hstack(
            (self.input_array[:, frame_count * self.cycle_count: frame_count * self.cycle_count + frames_left],
